...
define(['module'], function( module ){
	...
	text = {
		...
		load: function( name, req, onLoad, config ){
			...
			if( !hasLocation || useXhr(url, defaultProtocol, defaultHostName, defaultPort) ){
				...
				var onMessageReceived = function( event ){
					...
					if( appConfig.contentBase.indexOf(event.origin) == -1 && appConfig.resourceBase.indexOf(event.origin) == -1 ) return;

					var data = JSON.parse(event.data);
					if( data.action == 'loaded' ){
						event.source.postMessage(JSON.stringify({ action: "getTemplate", path: url }), "*");
					} else if( data.action == "setTemplate" ){
						text.finishLoad(parsed.moduleName + '.' + parsed.ext, parsed.strip, data.template, onLoad);
						if( window.removeEventListener ){
							window.removeEventListener("message", onMessageReceived);
						} else{
							window.detachEvent("onmessage", onMessageReceived);
						}
					}
				}

				var type = 'content';
				if( url.indexOf(appConfig.resourceBase) == 0 && appConfig.resourceBase != appConfig.contentBase ) type = 'resource';
				var hiddenIframe;

				if( document.getElementById('my_hidden_frame') && $(document.getElementById('my_hidden_frame')).attr('data-proxy') == type ){
					hiddenIframe = document.getElementById('my_hidden_frame');
				}
				...
				if( hiddenIframe ){
					hiddenIframe.contentWindow.postMessage(JSON.stringify({ action: "getTemplate", path: url }), "*");
					...
					// your ajax content
					hiddenIframe.src = (type == 'resource' ? appConfig.resourceBase : appConfig.contentBase) + "xs_template_proxy.html";
					$(hiddenIframe).attr('data-proxy', type);

					// give the frame an id
					hiddenIframe.id = 'my_hidden_frame';
		...
		writeFile: function( pluginName, moduleName, req, write, config ){
			var parsed = text.parseName(moduleName), nonStripName = parsed.moduleName + '.' + parsed.ext, //Use a '.js' file name so that it indicates it is a
